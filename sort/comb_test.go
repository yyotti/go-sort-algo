...
func TestCombSorter_Sort(t *testing.T) {
	tests := []sortTest{
		{
			src:      []Comparable{},
			expected: []Comparable{},
...
func BenchmarkCombSort_Sort64K(b *testing.B) {
	b.ResetTimer()

	b.StopTimer()
	for i := 0; i < b.N; i++ {
		sorter := NewCombSorter(slice64K)
...
func BenchmarkCombSort_AlmostSorted1K(b *testing.B) {
	...
	b.StopTimer()
	for i := 0; i < b.N; i++ {
		sorter := NewCombSorter(almostSorted1K)
...
func BenchmarkCombSort_AlmostSorted64K(b *testing.B) {
	b.ResetTimer()

	b.StopTimer()
	for i := 0; i < b.N; i++ {
		sorter := NewCombSorter(almostSorted64K)
